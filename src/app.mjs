...
function wireEvents() {
  ...
  els.settingsForm.addEventListener("submit", (event) => {
    ...
    settings = {
      ...
      ownerKey: els.ownerKeyInput.value.trim() || generateOwnerKey(),
...
function saveQuickTask() {
  ...
  const item = {
    ...
    category: expanded ? els.quickTaskCategory.value : "Personal",
    priority: expanded ? els.quickTaskPriority.value : "medium",
    due_date: expanded ? (els.quickTaskDate.value || todayKey()) : todayKey(),
    scheduled_at: null,
    duration_minutes: 30,
    completed: false,
    color: colorFor(expanded ? els.quickTaskCategory.value : "Personal"),
    source: "manual",
...
async function importCalendar() {
  ...
  try {
    ...
    if (fresh.length) {
      state.items = [...fresh, ...state.items.filter((item) => item.source !== "ics" || imported.some((candidate) => candidate.id === item.id))];
      persist();
      render();
...
function ensureOwnerKey(loadedSettings) {
  if (loadedSettings.ownerKey && loadedSettings.ownerKey.trim()) return loadedSettings;
  const next = { ...loadedSettings, ownerKey: generateOwnerKey() };
  saveJson(SETTINGS_KEY, next);
  return next;
}

function generateOwnerKey() {
  if (globalThis.crypto?.randomUUID) return `owner-${globalThis.crypto.randomUUID()}`;
